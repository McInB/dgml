...
def test_workspace_paths_are_derived_from_keys(tmp_path: Path) -> None:
    """Every ``Workspace`` path helper must resolve to its key's location — this
    is the drift the shared layout exists to prevent."""
    ws = Workspace(root=tmp_path)
    cases = [
        (ws.file_dir("f1"), layout.file_prefix("f1")),
        (ws.file_pages_dir("f1"), layout.file_pages_prefix("f1")),
        (ws.file_text_dir("f1"), layout.file_text_prefix("f1")),
        (ws.file_json_path("f1"), "files/f1/file.json"),
        (ws.file_errors_path("f1"), "files/f1/errors.json"),
        (ws.docset_dir("d1"), layout.docset_prefix("d1")),
        (ws.docset_files_dir("d1"), layout.docset_files_prefix("d1")),
        (ws.docset_json_path("d1"), "docsets/d1/docset.json"),
        (ws.docset_schema_path("d1"), layout.docset_extraction_schema_key("d1")),
        (ws.docset_full_schema_path("d1"), layout.docset_full_schema_key("d1")),
        (ws.docset_generation_schema_path("d1"), layout.docset_generation_schema_key("d1")),
        (ws.docset_file_dir("d1", "f1"), layout.docset_pair_prefix("d1", "f1")),
        (ws.file_dgml_xml_path("d1", "f1", "r"), layout.dgml_xml_key("d1", "f1", "r")),
        (
            ws.docset_file_extraction_stats_path("d1", "f1"),
            "docsets/d1/files/f1/extraction_stats.json",
        ),
    ]
    for path, key in cases:
        assert path == tmp_path / key.rstrip("/"), key
