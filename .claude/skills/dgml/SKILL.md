...
Managing multiple workspaces: `dgml workspace list` prints every workspace the store holds (`{workspace_id, name, organization, storage_service, root, created_at}`), and any `workspace_id` opens it from anywhere — e.g. `id=$(dgml workspace create --organization Acme | jq -r .workspace_id)` then `dgml --workspace "$id" file list`. Every field is derived from each workspace's own config, so a row cannot disagree with the workspace it describes. A **detached** workspace is not in the store and so not listed; `dgml workspace import <path>` adds one, and `dgml workspace import` with no arguments sweeps every workspace an older dgml left in `~/.config/dgml/workspaces.json` (data never moves — the existing directory is recorded as `workspace_path`, which does not re-seal the workspace). (`dgml workspace register` has been removed.)
